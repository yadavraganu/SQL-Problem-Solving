...
Each row in this table indicates that some post was removed as a result of being reported or as a result  
of an admin review.
 
Write an SQL query to find the average for daily percentage of posts that got removed after being reported  
...
3. DISPLAY INPUT DATA
*******************************************************************************/
SELECT * FROM ACTIONS ORDER BY POST_ID, USER_ID;
SELECT * FROM REMOVALS ORDER BY POST_ID;
GO
/*******************************************************************************
4. SOLUTION
*******************************************************************************/
WITH T AS (
  SELECT
    CAST(COUNT(DISTINCT T2.POST_ID) AS FLOAT) / NULLIF(COUNT(DISTINCT T1.POST_ID), 0) * 100 AS PCT
  FROM ACTIONS AS T1
...
SELECT
  ROUND(AVG(PCT), 2) AS AVERAGE_DAILY_PERCENT
FROM T;
```
...
after all described matches. Result table should be ordered by num_points (decreasing order).  
...
The query result format is in the following example:Dictionaries & Encyclopedias
...
SELECT VISITED_ON,AMOUNT,AVERAGE_AMOUNT FROM RUNNING_AVG WHERE DAY_NUM >= 7
ORDER BY 1 ASC
```
...
The query result format is in the following example:Mathematics
...
3. DISPLAY INPUT DATA
*******************************************************************************/
SELECT * FROM STOCKS ORDER BY OPERATION_DAY;
GO
/*******************************************************************************
4. Solution
*******************************************************************************/
SELECT
STOCK_NAME,
SUM(CASE WHEN UPPER(OPERATION) = 'BUY' THEN PRICE * -1 ELSE PRICE END) AS CAPITAL_GAIN_LOSS
FROM STOCKS
GROUP BY STOCK_NAME
ORDER BY 2 DESC
```
...
SELECT 
SALE_DATE, 
SUM(CASE WHEN UPPER(FRUIT) = 'APPLES' THEN SOLD_NUM ELSE 0 END) -
SUM(CASE WHEN UPPER(FRUIT) = 'ORANGES' THEN SOLD_NUM ELSE 0 END) AS DIFF 
FROM SALES 
GROUP BY SALE_DATE 
...
| 8              |
+----------------+
"2021-04-03"  --> We have one transaction with ID 8, so we add 8 to the result table.

"2021-04-28" --> We have two transactions with IDs 5 and 9. The transaction with ID 5 has an  
amount of 40, while the transaction with ID 9 has an amount of 21. We only include the transaction  
with ID 5 as it has the maximum amount this day.

"2021-04-29" --> We have two transactions with IDs 1 and 6. Both transactions have the same amount  
of 58, so we include both in the result table.
...
4. SOLUTION
*******************************************************************************/
WITH MAX_AMOUNT_AT_DAY AS (
SELECT *,
MAX(AMOUNT) OVER (PARTITION BY FORMAT(DAY,'yyyy-MM-dd')) AS MAX_AMT_AT_DAY
FROM TRANSACTIONS
)
SELECT TRANSACTION_ID FROM MAX_AMOUNT_AT_DAY WHERE MAX_AMT_AT_DAY = AMOUNT
...
SELECT DEPARTMENT,EMPLOYEE,SALARY FROM RANKED_EMP WHERE RNK = 1 ;
----------------------------------------------------------------
WITH
...
transaction_id is the primary key for this table.
Each row contains information about one transaction.
type is ENUM ('Creditor','Debtor') where 'Creditor' means the user deposited money into their  
...
consecutive months. The total income of an account in some month is the sum of all its deposits in that  
month (i.e., transactions of the type 'Creditor').
...
We can see that the income exceeded the max income of 21000 for two consecutive months, so we include 3  
in the result table.
...
We can see that the income exceeded the max income in May and July, but not in June. Since the account did not  
...
- A team’s ID is assigned based on the rank of the team’s salary relative to the other teams’ salaries, where  
  the team with the lowest salary has team_id = 1. Note that the salaries for employees not on a team are not  
...
Each row of this table contains information about the account and the date associated with a stream  
session.
 
Write an SQL query to report the number of accounts that bought a subscription in 2021 but did not  
have any stream session.
...
Write an SQL query to report the balance of each user after each transaction. You may assume  
that the balance of each account before any transaction is 0 and that the balance will never  
be below 0 at any moment.
...
Each row of this table indicates that there were flights_count flights that departed from departure_airport  
and arrived at arrival_airport.
 
Write an SQL query to report the ID of the airport with the most traffic. The airport with the most traffic is  
the airport that has the largest total number of flights that either departed from or arrived at the airport.  
...
4. Solution
*******************************************************************************/
WITH NUM_OF_TRANS_AND_PRE_SPENDS AS (
SELECT *,
...
LAG(SPEND,2) OVER (PARTITION BY USER_ID ORDER BY TRANSACTION_DATE) AS PREV_TO_PREV_SPEND
FROM TRANSACTIONS
)
SELECT USER_ID,
SPEND AS THIRD_TRANSACTION_SPEND,
TRANSACTION_DATE AS THIRD_TRANSACTION_DATE FROM NUM_OF_TRANS_AND_PRE_SPENDS 
WHERE TRANS_NO = 3 AND SPEND > PREV_TO_PREV_SPEND AND SPEND > PREV_SPEND
```
...
student_id 321 has the highest score of 230, while student_id  
896 has the lowest score of 119. Therefore, the difference between them is 111.
```
```sql
/*******************************************************************************
1. SETUP: CLEAN UP AND RECREATE TABLE
...
Return the result table ordered by type, duration, and first_name in descending order and  
...
3. DISPLAY INPUT DATA
*******************************************************************************/
SELECT * FROM CINEMA ORDER BY SEAT_ID;
GO
/*******************************************************************************
4. Solution
*******************************************************************************/
...
Write a solution to find the total parking fee paid by each car across all parking lots, and the average hourly 
fee (rounded to 2 decimal places) paid by each car. Also, find the parking lot where each car spent the most  
total time.

Return the result table ordered by car_id in ascending order.

Note: Test cases are generated in such a way that an individual car cannot be in multiple parking lots at the  
same time.
The result format is in the following example.
Example:
...
CASE
    WHEN POSITION <= CEILING(TOTAL_TEAMS / 3.0) THEN 'TIER 1'
    WHEN POSITION <= CEILING(2 * TOTAL_TEAMS / 3.0) THEN 'TIER 2'
    ELSE 'TIER 3'
END TIER
FROM SCORED_TEAM
ORDER BY 2 DESC, 1;
...
This table contains information about the shifts worked by employees, including the start and  
end times on a specific date.
...
employee who has at least one overlapping shift, ordered by employee_id in ascending order.
```
```sql
/*******************************************************************************
1. SETUP: CLEAN UP AND RECREATE TABLE
...
3. DISPLAY INPUT DATA
*******************************************************************************/
SELECT * FROM EMPLOYEESHIFTS ORDER BY EMPLOYEE_ID, START_TIME;
GO
/*******************************************************************************
4. SOLUTION
*******************************************************************************/
...
- For product 1, it belongs to the Electronics category which has a 10% discount, so the final price  
  is 1000 - (10% of 1000) = 900.
- For product 2, it belongs to the Clothing category which has a 20% discount, so the final price  
  is 50 - (20% of 50) = 40.
- For product 3, it belongs to the Electronics category and receives a 10% discount, so the final  
  price is 1200 - (10% of 1200) = 1080.
...
SELECT 
    P.PRODUCT_ID,
    P.PRICE - (P.PRICE * COALESCE(D.DISCOUNT, 0) / 100.0) AS FINAL_PRICE,
    P.CATEGORY
FROM PRODUCTS P
...
Return the result table ordered by the count of matching-letter cities in descending order and  
then by state name in ascending order.
...
3. DISPLAY INPUT DATA
*******************************************************************************/
SELECT * FROM CITIES ORDER BY STATE, CITY;
GO
/*******************************************************************************
4. Solution
*******************************************************************************/
...
SUM(CASE WHEN LEFT(UPPER(STATE),1) = LEFT(UPPER(CITY),1) THEN 1 ELSE 0 END) AS MATCHING_LETTER_COUNT
FROM CITIES 
GROUP BY STATE 
HAVING SUM(CASE WHEN LEFT(UPPER(STATE),1) = LEFT(UPPER(CITY),1) THEN 1 ELSE 0 END) > 1
ORDER BY 3 DESC,1
```
